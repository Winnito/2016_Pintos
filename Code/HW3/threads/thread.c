...
/* Init scheduler variable */
#define NICE_DEFAULT 0
#define RECENT_CPU_DEFAULT 0
#define LOAD_AVG_DEFAULT 0

/* load average count of process in 1 min */
int load_avg;
               ...
               thread_func *function, void *aux) 
{
  ...
  t->load_succeed = 0;             /* not load program */
  t->exit_process = 0;             /* not exit process */ 

  list_init(&t->child_list);
  sema_init(&t->sema_exit,0);      /* exit semaphore 0 */
  sema_init(&t->sema_load,0);      /* load semaphore 0 */   

  /* Add child_element to current process's child list */ 
...
thread_unblock (struct thread *t) 
{
  ...
  ASSERT (t->status == THREAD_BLOCKED);
	...
	list_insert_ordered(&ready_list,&t->elem,cmp_priority,0);	/* Sort by priority in ready list */
...
thread_exit (void) 
{
  ...
  //t->exit_process = 0;
  sema_up(&t->sema_exit);     /* exit status exit */
...
/* Compare priority the most thread in ready_list to current thread */
void test_max_priority(void) 
{
	enum intr_level old_level;
	old_level = intr_disable();

	/*Check empty and Compare current thread to first thread in ready_list */
...
{	
	...
	int p1 = div_mixed(t->recent_cpu,4);
	int p2 = mult_mixed(int_to_fp(t->nice),2);
	priority = sub_fp(priority,p1);
...
void mlfqs_recent_cpu(struct thread *t) 
{
	...
	int c1 = mult_mixed(load_avg,2);
	int c2 = add_mixed(mult_mixed(load_avg,2),1);
	int c3 = t->recent_cpu;
	int c4 = int_to_fp(t->nice);
...
void mlfqs_load_avg(void) 
{
	...
	int a2 = load_avg;
	int a3 = div_fp(int_to_fp(1),int_to_fp(60));
	int a4 = int_to_fp(ready_count());
	
	load_avg = add_fp(mult_fp(a1,a2),mult_fp(a3,a4));	
